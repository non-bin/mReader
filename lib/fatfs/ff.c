/**
 * \brief FatFs - Generic FAT Filesystem module  R0.16
 *
...
#if FF_REVISION_ID != 80386 /* Revision ID */
...
)
{
	...
	if (sdir)
	{ /* Initialize the containing directory. This block needs to precede the followings. */
...
)
{
	...
	while (dp->sect)
	{
		...
		{																										 /* On the FAT/FAT32 volume */
			...
			if (et == DDEM || et == '.' || (int)((attr & ~AM_ARC) == AM_VOL) != vol)
			...
			if (et != DDEM && et != '.' && attr != AM_LFN && (int)((attr & ~AM_ARC) == AM_VOL) == vol)
			{ /* Is it a valid entry? */
				break;
			}
#endif
	...
	{															 /* On the exFAT volume */
		...
		{ /* Has the directory been stretched by new allocation? */
			...
			{ /* Is it a sub-directory? */
				...
				for (n = 1; n <= fs->xcwds.depth && dp->obj.sclust != fs->xcwds.tbl[n].d_scl; n++)
...
)
{
	...
	{ /* If LFN is invalid, altname[] needs to be copied to fname[] */
		if (di == 0)
		{													 /* If LFN and SFN both are invalid, */
...
#if FF_FS_RPATH
	if (fs->fs_type == FS_EXFAT)
	{ /* exFAT: Retrieve the start-directory's status */
		if (dp->obj.sclust)
		{ /* Start directory is a sub-directory */
		...
		{ /* Start directory is the root directory */
		...
		for (;;)
		{
			...
			{ /* Is it a dot name? */
				...
				{											/* ".." in the sub-dir? */
					...
					dp->obj.objsize = fs->xcwds2.tbl[fs->xcwds2.depth].d_size & 0xFFFFFF00;
					if (fs->xcwds2.depth)
					{ /* Load containing dir info if needed */
						dp->obj.c_scl = fs->xcwds2.tbl[fs->xcwds2.depth - 1].d_scl;
			...
			{ /* Failed to find the object */
				...
				{ /* Object is not found */
					...
					{ /* If dot entry is not exist, stay there (may be root dir in FAT volume) */
						if (!(ns & NS_LAST))
...
#if FF_FS_EXFAT && FF_FS_RPATH
			if (fs->fs_type == FS_EXFAT && (dp->obj.attr & AM_DIR))
			{ /* Record the path if it is a sub-directory */
				fs->xcwds2.tbl[fs->xcwds2.depth].nxt_ofs = dp->blk_ofs;
				if (++fs->xcwds2.depth >= sizeof fs->xcwds2.tbl / sizeof fs->xcwds2.tbl[0])
				{ /* Is it too deep path? */
...
)
{
	...
	if (FF_MULTI_PARTITION && part > 4)
		return 3; /* MBR has four primary partitions max (FatFs does not support logical partition) */
	for (i = 0; i < 4; i++)
...
)
{
	...
	if (cfs)
	{ /* Unregister current filesystem object */
		FatFs[vol] = 0;
...
)
{
	...
	if (res == FR_OK)
	{
		...
		if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW))
		{
			...
			{ /* An object with the same name is already existing */
...
)
{
	...
	if (res == FR_OK)
	{
		...
		{ /* On the exFAT volume */
			...
			if (wi < len)
				buff[wi++] = '/';
			for (di = 0; wi < len && di < fs->xcwds.depth; di++)
			{																					 /* Follow current directory path with cwd structure */
				...
				get_fileinfo(&dj, &fno);
				if (di > 0 && wi < len)
					buff[wi++] = '/'; /* Add the directory name with a directory separator */
		...
		{ /* On the FAT/FAT32 volume */
			...
			{																	/* Repeat while current directory is a sub-directory */
				...
				if (res != FR_OK)
					break;
				do
				{ /* Find the entry links to this sub-directory */
...
)
{
	...
	if (res == FR_OK)
	{
		...
		{ /* Follow completed */
			...
			{ /* It is neither the origin directory itself nor dot name in exFAT */
				if (dp->obj.attr & AM_DIR)
...
)
{
	...
	if (res == FR_OK)
	{
		...
		res = follow_path(&dj, path); /* Follow the file path */
		if (res == FR_OK)
		{ /* Invalid name or name collision */
...
)
{
	...
	if (res == FR_OK)
	{
		...
		if (res == FR_OK)
		{ /* It is ready to rename the object */
#if FF_FS_EXFAT
			if (fs->fs_type == FS_EXFAT)
			{ /* At exFAT volume */
#if FF_FS_RPATH
			...
			{																		 /* At FAT/FAT32 volume */
				...
				{														/* It is a valid path and no name collision */
					...
					if (res == FR_OK)
					{
						...
						{ /* Update .. entry in the sub-directory being moved if needed */
...
)
{
	...
	if (res == FR_OK)
	{
		...
		if (res == FR_OK)
		{
			...
			{ /* On the exFAT volume */
				if (fno->fdate)
				{ /* Change last modified time if needed */
